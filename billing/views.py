    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                ...
                change_to_give = int((purchase.paid_amount - purchase.total).quantize(Decimal('1.'), rounding=ROUND_HALF_UP)) if purchase.paid_amount > purchase.total else 0
                ...
                if change_to_give > 0:
                    for denom in Denomination.objects.all().order_by('-value'):
                        ...
                        if give > 0:
                            ChangeDenomination.objects.create(purchase=purchase, denomination_value=denom_value, count_given=give)
            ...
            messages.error(request, f"Failed to create purchase: {e}")
            return render(request, "billing/billing_page.html", {"products": products, "denominations": denominations})
...
def invoice_page(request, purchase_id):
    ...
    change_list = purchase.change_denominations.all()
    return render(request, "billing/invoice.html", {"purchase": purchase, "change_list": change_list})
